...
export default class extends Base {
   ...
   */
  async seckill_show_prize_listGET(getData) {
    const { page, limit } = getData;
    const where = {};
    const sort = 'id,DESC'
    const likeRuleList = await this.model('seckill_show_prize').adminpage(page, limit, where, sort);
    return {
   ...
   * @returns {Promise<*>}
   */
  async get_seckill_show_prizeGET({ id }) {
    return  await this.model('seckill_show_prize').where({ id }).find()
   ...
   */
  async seckill_show_prize_addPOST({},postData){
    const coverImg = this.saveBase64Image(postData.head_img, 'show_seckill_head')
    const addData = {
    ...
    const add = await this.model('seckill_show_prize').add(addData);
    addData.id = add
    try {
      await GRedis.hset('SEC_KILL_MOCK_USER',add,JSON.stringify(addData))
    } catch (err) {
       console.log('redis add prize error')
    }
   ...
   * @returns {Promise<{status: string}>}
   */
  async seckill_show_prize_updatePOST({}, postData) {
    let coverImg = '';
    if (postData.form.head_img.slice(0, 10) == 'data:image') {
      coverImg = this.saveBase64Image(postData.form.head_img, 'show_seckill_head')
      // 删除原来的图片
      const row = await this.model('seckill_show_prize').where({ id: postData.id }).find();
    ...
    const update = await this.model('seckill_show_prize').where({ id: postData.id }).update(updateData)
    updateData.id = postData.id
    try {
      await GRedis.hset('SEC_KILL_MOCK_USER',postData.id,JSON.stringify(updateData))
    } catch (err) {
      console.log('redis add prize error')
    }
    if (update) {
   ...
   * @returns {Promise<{status: string}>}
   */
  async seckill_show_prize_update_oldPOST({}, { id, tel, nickname, money }) {
    if (!tel || !nickname || !id) {
      return { status: 'fail' }
    }
    const update = await this.model('seckill_show_prize').where({ id }).update({
      tel, nickname, money
    })
    if (update) {
      return { status: 'ok' }
   ...
   * @returns {Promise<*>}
   */
  async seckill_show_prize_add_manyPOST({}, { contents }) {
    if (!contents) {
      return { status: 'fail' }
    }
    ...
    if (addData.length > 0) {
      await this.model('seckill_show_prize').addMany(addData)
      return { status: 'ok' }
    }
    return { status: 'fail' }
  }
   ...
   * @returns {Promise<*>}
   */
  async seckill_show_prize_add_manyBAKPOST({}, { contents }) {
    if (!contents) {
      return { status: 'fail' }
    }
    const addData = contents.split(/[\n]/);
    if (addData.length > 0) {
      await this.model('seckill_show_prize').addMany(addData.map(i => {
        return { content: i }
      }))
      return { status: 'ok' }
    }
   ...
   * @returns {Promise<*>}
   */
  async seckill_show_prize_replace_allPOST({}, { contents }) {
    if (!contents) {
      return { status: 'fail' }
    }
    const addData = contents.split(/[\n]/);
    if (addData.length > 0) {
      await this.model('seckill_show_prize').delete();
      await this.model('seckill_show_prize').addMany(addData.map(i => {
        return { content: i }
      }))
      return { status: 'ok' }
   ...
   */
  async seckill_show_prize_deletePOST({}, { id }) {
    const del = await this.model('seckill_show_prize').where({ id }).delete();
    try {
      await GRedis.hdel('SEC_KILL_MOCK_USER',id)
    } catch (err) {
      console.log('redis add prize error')
    }
    if (del) {
